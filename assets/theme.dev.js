...
(function (scrollLock) {
  ...
  class QuantityInput extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.quantityUpdateUnsubscriber = subscribe(
        theme.PUB_SUB_EVENTS.quantityUpdate,
  ...
  class CartItems extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.freeShippingLimit = this.freeShipping.length
        ? Number(this.freeShipping[0].getAttribute(attributes$d.freeShippingLimit)) * 100 * window.Shopify.currency.rate
        : 0;
    ...
    updateCart(updateData = {}, currentItem = null) {
      ...
      const lineIndex = newItem?.hasAttribute(attributes$d.itemIndex)
        ? parseInt(newItem.getAttribute(attributes$d.itemIndex))
        : 0;
      const itemTitle = newItem?.hasAttribute(attributes$d.itemTitle)
        ? newItem.getAttribute(attributes$d.itemTitle)
        : null;
    ...
    hideAddToCartErrorMessage() {
      const holder = this.button.closest(selectors$l.upsellHolder)
        ? this.button.closest(selectors$l.upsellHolder)
        : this.button.closest(selectors$l.productForm);
    ...
    addToCartError(data, button) {
      ...
      if (button !== null) {
        const outerContainer =
          button.closest(selectors$l.outerSection) ||
          button.closest(selectors$l.quickAddHolder) ||
          button.closest(selectors$l.quickAddModal);
      ...
      setTimeout(() => {
        buttons.forEach((button) => {
          ...
          const isVariantUnavailable =
            button.closest(selectors$l.formWrapper)?.classList.contains(classes$i.variantSoldOut) ||
            button.closest(selectors$l.formWrapper)?.classList.contains(classes$i.variantUnavailable);
    ...
    build(data) {
      ...
      this.newTotalItems =
        cartItemsData && cartItemsData.querySelectorAll(selectors$l.item).length
          ? cartItemsData.querySelectorAll(selectors$l.item).length
          : 0;
      this.subtotal =
        cartTotal && cartTotal.hasAttribute(attributes$d.cartTotal)
          ? parseInt(cartTotal.getAttribute(attributes$d.cartTotal))
          : 0;
      this.cartCount = this.getCartItemCount();
    ...
    getCartItemCount() {
      return Array.from(this.cart.querySelectorAll(selectors$l.qtyInput)).reduce(
  ...
  class CartDrawer extends HTMLElement {
    ...
    openCartDrawerOnSelect(e) {
      const cartDrawerSection =
        e.target.querySelector(selectors$k.shopifySection) || e.target.closest(selectors$k.shopifySection) || e.target;
      ...
      class HeaderMobileSliderule extends HTMLElement {
        ...
        hideSliderule(close = false) {
          ...
          const hiddenSelector = close
            ? `[${selectors$g.animates}].${classes$c.isHidden}`
            : `[${selectors$g.animates}="${newPosition}"]`;
          ...
          const children = close
            ? this.pane.querySelectorAll(`.${classes$c.isVisible}, .${classes$c.isHiding}`)
    ...
    customElements.define(
      ...
      class PredictiveSearch extends HeaderSearchForm {
        ...
        switchOption(direction) {
          ...
          const allVisibleElements = Array.from(this.querySelectorAll(selectors$a.allVisibleElements)).filter(
    ...
    customElements.define(
      ...
      class ProductImages extends HTMLElement {
        ...
        toggleEvents() {
          ...
          if (
            (isMobileView && this.hasAttribute(attributes$5.faderMobile)) ||
            (!isMobileView && this.hasAttribute(attributes$5.faderDesktop))
          ) {
        ...
        getNextMediaId() {
          ...
          const nextMedia = currentMedia?.nextElementSibling.hasAttribute(attributes$5.imageId)
            ? currentMedia?.nextElementSibling
            : this.querySelector(selectors$6.productMedia);
  ...
  class SiblingSwatches {
    ...
    cacheDefaultValues() {
      this.activeSibling = this.siblingsInnerHolder
        .querySelector(selectors$4.siblingLinkCurrent)
        ?.closest(selectors$4.siblingLink);
    ...
    showSiblingImage(siblingImage) {
      ...
      const swapImages = () => {
        ...
        const swapActiveSiblingImage =
          siblingImage === activeSiblingImage && this.productImageSibling.classList.contains(classes$4.visible);
